  ...
  /**
    ...
    */
        ...
        throws ServletException, IOException
    {
        ...
        String portalactions="sampleportal samplesetup  sampleofficeview travelschedule travelbooking traveladmin";
        ...
        */
        ...
        if(!tu.isEmptyValue(servicekey)){
        	try {
				...
				if(!tu.isEmptyValue(val)){
					...
					if(!tu.isEmptyValue(action) &&action.indexOf("-")>=0){
						String[] keys=action.split("-");
						if(keys.length>=2){
							useraction=keys[0];
							if(!tu.isEmptyValue(keys[1])&& keys[1].length()==32){
								refobjid= action;
							}
						}
					}
        ...
        }else if(access!=null && !tu.isEmptyValue(useraction) && portalactions.contains(useraction)){            
               logger.info("\n"+date.toString()+" Loged in User:="+username+" "+":-Assigned Token="+access.getToken() +":-refobjid="+refobjid);
               //access token is appended with Client IP in the indexpage
               strNextPage="/src/portal/"+useraction+".jsp?username="+username+"&refobjid="+refobjid+"&referer="+referer+"&setter="+setter
            		   +"&usertoken="+access.getToken()+CONST.IPSEPERATOR+remoteaddress
            		   +CONST.IPSEPERATOR+username+"&baseurl="+baseurl+"&mqid="+access.getMessageque(); 
