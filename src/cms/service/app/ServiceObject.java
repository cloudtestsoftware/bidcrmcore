...
public String getChildButtonAction(String table){
		...
		if(props!=null &&props.getRowCount()>0){
			for(int row=0;row<props.getRowCount();row++){
				...
				if(button.length>1 &&button[0].toLowerCase().contains("button")){
					String button_id=table.toLowerCase()+":"+props.getFieldValue("Name", row).toLowerCase()+":"+props.getFieldValue("PropertyValue", row);
					
					action+="{ \"id\":\""+button_id+"\", \"text\":\""+button[1]+"\"  },";
	...
	public String getChildButtonAction_old(String table){
		...
		if(props!=null &&props.getRowCount()>0){
			for(int row=0;row<props.getRowCount();row++){
				...
				if(button.length>1 &&button[0].toLowerCase().contains("button")){
					...
					action+="<item type=\"button\" id=\""+button_id+"\" text=\""+button[1]+"\"  img=\"red.gif\"/><item type=\"separator\" id=\"button_separator_"+row+"\" />";
			...
			for(int row=0;row<props.getRowCount();row++){
				...
				if(button.length>1 &&button[0].toLowerCase().contains("button")){
					...
					if(tu.isEmptyValue(action)){
						action+="{ type:\"button\" , name:\""+button_id+"\", value:\""+button[1]+"\", command:\""+button_id+"\"  }";
					}else{
						action+=",\n{ type:\"button\" , name:\""+button_id+"\", value:\""+button[1]+"\", command:\""+button_id+"\"  }";
					}
				}
