...
public class TemplateUtility {
	...
	/*public Rows getXMLRows(TemplateTable tab, String table,String codefieldlist[],String propfieldlist[],String relationlist[],
			...
			*/
	public Rows getXMLRows(TemplateTable tab, String table,String codefieldlist[],String propfieldlist[],String relationlist[],
			String captions[],String[] datadomain,QueryImpl impldata){


		Rows rows=null;
		Head head=null;
		HashMap<String,ArrayList<Option>> codes=null;
		HashMap<String,ArrayList<Option>> props=this.getProperty(table, propfieldlist,impldata.getGroupuser());
		HashMap<String,ArrayList<Option>> relationprops=this.getRelationOptions(relationlist, impldata);
        ...
        if(exceptiontables.contains(table.toLowerCase())){
        	codes=this.getCodeObject(table, codefieldlist, impldata);
		...
		//if(tab.getRowCount()>0){
			...
			for (String colname:tab.getColumnNames()){
				...
				if(caption.equalsIgnoreCase("messagequeue")){
					caption="AssignTo";
				}
	...
	public Rows getXMLFilterRows(TemplateTable tab, String table,String codefieldlist[],String propfieldlist[],
			String relationlist[],String columns[], String captions[],String[] datadomain,QueryImpl impldata){
	 ...
	 * @table : actual table name 
	 */
	public Items getXMLForm(TemplateTable tab, String table,String codefieldlist[],String propfieldlist[],String relationlist[], 
			String captions[],String[] datadomain,String[] formfields,QueryImpl impldata){
		
        String exceptiontables="sampleorder";
		//Rows rows=null;
		Items items=null;
		HashMap<String,ArrayList<Option>> codes=null;
		HashMap<String,ArrayList<Option>> props=this.getProperty(table, propfieldlist,impldata.getGroupuser());
		HashMap<String,ArrayList<Option>> relationprops=this.getRelationOptions(relationlist, impldata);
	    String inputwidth="200";
        codes=this.getCodeObject(table, codefieldlist, impldata);
        	//this.getCodeObject(table, codefieldlist, groupuser);
        //logger.info("codes="+codes.values());
		...
		//if(tab.getRowCount()>0){
			...
			item.setLabelWidth("150");
			item.setInputWidth(inputwidth);
			...
			for (String colname:tab.getColumnNames()){
				...
				if(colindex>=0){
					//System.out.println(colname+"="+meta.getColumnNullables()[colindex]);
					colsize=Integer.parseInt(meta.getColumnSizes()[colindex]);
					String datatype=meta.getColumnDataTypes()[colindex];
					String nullable=meta.getColumnNullables()[colindex].equals("0")?"false":"true";
					if(nullable.equalsIgnoreCase("true") &&datatype.toLowerCase().contains("char")){
						int index=tab.getColumnIndex(colname);
					...
					if(colname.equalsIgnoreCase("objid")){
						...
						item.setType("editor");
						if(exceptiontables.contains(table.toLowerCase())) {
							item.setInputWidth("200");
						}else {
							item.setInputWidth("565");
						}
					
						item.setInputHeight("100");
						//item.setToolbar("false");
				...
				if(!exceptiontables.contains(table.toLowerCase())) {
					if(!ishidden && (colcount+1)%2!=0 ||colsize>300) {
						Item newcol= new Item();
						newcol.setType("newcolumn");
						newcol.setOffsetLeft("50");
						itemlist.add(newcol);
					}
				}
				itemlist.add(item);
	...
	public Rows getXMLConsoleRows(String username ){
		...
		String sql= "select c.* from table_console c, table_messagequeue mq ,table_user u where "+
				"  mq.objid=c.mqid  and u.objid=mq.messagequeue2user and upper(u.loginname)= upper('"+username+"') "
						+ " and nvl(c.isarchived,0)='0' order by c.name";
						*/
		String sql= "select distinct lower(c.name) name,count(c.objid) as count from table_console c, table_user u where "+
				"   upper(u.loginname)= upper('"+username+"') "
						+ " and nvl(c.isarchived,0)='0' and upper(c.genuser)= upper(u.loginname)"
						+ "and c.mqid is not null group by c.name order by lower(c.name)";
        ...
        String oldname="";   
			for (int i=0; i<tab.getRowCount(); i++){
				...
				if(this.isEmptyValue(oldname) &&!isEmptyValue(name)||!oldname.equalsIgnoreCase(name)){
					...
					master.setId(name+"s");
					List<String> cells =new ArrayList<String>();
					cells.add(tab.getFieldValue("name",i)+"("+tab.getFieldValue("count",i)+")");
					cells.add("Record count="+tab.getFieldValue("count",i));
	...
	public Rows getXMLConsoleRows_old(String username ){
		...
		String sql= "select c.* from table_console c, table_messagequeue mq ,table_user u where "+
				"  mq.objid=c.mqid  and u.objid=mq.messagequeue2user and upper(u.loginname)= upper('"+username+"') "
						+ " and nvl(c.isarchived,0)='0' order by c.name";
						*/
		String sql= "select c.* from table_console c, table_user u where "+
				"   upper(u.loginname)= upper('"+username+"') "
						+ " and nvl(c.isarchived,0)='0' and upper(c.genuser)= upper(u.loginname)"
						+ "and c.mqid is not null order by c.name";
		TemplateTable tab=this.getResultSet(sql);
        String oldname="";   
			for (int i=0; i<tab.getRowCount(); i++){
				String name=tab.getFieldValue( "name",i);
	...
	private String dhtmlxValidator(String domain){
		if(domain.toLowerCase().contains("raw_")){
	...
	public HashMap<String,ArrayList<Option>> getCodeObject(String table, String fieldlist[],QueryImpl impldata){
		return new CodeManager().getCodeObject(table, fieldlist, impldata);
	}
	...
	public HashMap<String,ArrayList<Option>> getRelationOptions (String relationlist[],QueryImpl impldata)
	{
		...
		for(String relation:relationlist){
		    ...
		    if(tables.length>=3 && tables[2].equals("list")){
		    	...
		    	String sql="";
		    	String filterVal="";
		    	//if(tables.length==4 &&!this.isEmptyValue(impldata.getBqn()) &&!this.isEmptyValue(tables[3]) &&tables[3].toLowerCase().contains("select")){
		    	if(tables.length==4  &&!this.isEmptyValue(tables[3]) &&tables[3].toLowerCase().contains("select")){
			    	sql=tables[3].replaceAll("\\^", "'"); 	
		    		sql=sql.replaceAll("@bqn", "'"+impldata.getBqn()+"'") +" order by 2";
		    		sql=sql.replaceAll("@groupuser",  "'"+impldata.getGroupuser()+"'");
		    		sql=sql.replaceAll("@genuser",  "'"+impldata.getUsername()+"'");
		    		sql=sql.replaceAll("@parentobjid",  "'"+impldata.getParentobjid()+"'");
		    	}else if(!this.isEmptyValue(impldata.getBqn())&&impldata.getBqn().length()>=5){
		    		sql= "select *from table_"+tables[0]+ " where groupuser='"+impldata.getGroupuser()+"' and upper(bqn)=upper('"+impldata.getBqn()+"') or bqn is null order by 2";
		    		...
		    		sql= "select *from table_"+tables[0]+ " where groupuser='"+impldata.getGroupuser()+"' order by 2";
		    	}
		    	TemplateTable prop=this.getResultSet(sql);
		            ...
		            if( prop.getRowCount()>0){
						for(int n = 0; n < prop.getRowCount(); n++){
							...
							}else if(tables[0].toLowerCase().equals("user")){
									optionval.add(prop.getFieldValue("name", n)+ " "+prop.getFieldValue("lastname", n)+ " ("+prop.getFieldValue("loginname", n)+")");
							}else{
				...
				if(consoledata.getRowCount()>0 && isParent){
					...
					if(tabledata.getFieldValue("status",i).equals(ApplicationConstants.STATUS_UNASSIGN)||
						...
						updatesql= "update table_console set mqid='"+tabledata.getFieldValue(objectname+"2messagequeue",i)+
								...
								"',status='"+tabledata.getFieldValue("status",i)+"',moduser='"+username+"',moddate="+(dbtype.equalsIgnoreCase("Oracle")? "sysdate":"getdate()")+
					...
					insertsql="insert into table_console(OBJID,NAME,TITLE,DESCRIPTION,STATUS,KEYOBJID,ENTRYDATE,"+
							...
							tabledata.getFieldValue("objid",i)+"',"+
