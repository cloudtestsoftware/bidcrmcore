...
public class TemplateQuery extends ParseQuery{
	...
	public void makeTableSelect(String table,String criteria , String operator ,String value,String accessfilter,QueryImpl impldata)

	{
		...
		search=this.getAccessFilter(table,search,accessfilter, impldata,"");
	...
	public void makeTableSelect(String table,String criteria , String operator ,String value,String [] column,String[] datatype,String accessfilter,QueryImpl impldata)
	...
	public void makeSqlSelect(String sql,String criteria , String operator ,String value,String orderfields,String table,String accessfilter,QueryImpl impldata)

	{
		...
		if(sql.toLowerCase().indexOf(table.toLowerCase())>=0){
			search=this.getAccessFilter(table,search,accessfilter, impldata,table.toLowerCase());
		}else{
			search=this.getAccessFilter(table,search,accessfilter, impldata,"");
		}
	...
	public String getAccessFilter(String table,String search, String acessfilter,QueryImpl impldata,String tablealias){
		if(!tu.isEmptyValue(tablealias)){
			tablealias=tablealias+".";
		}
		if(tu.isEmptyValue(acessfilter)) return search;
		
		String filter="(upper("+tablealias+"genuser)= upper('"+impldata.getUsername()+"') "+
				" or "+tablealias+"objid in ("+
				acessfilter.replaceAll("@groupuser", "'"+impldata.getGroupuser()+"'")+"))";
		if(tu.isEmptyValue(search)) 
	...
	public  String  makeChildSql(String parent,String childname,String relfield,String pid,String[]column,String[]datatype,String accessfilter,QueryImpl impldata){
		String dbtype=DatabaseTransaction.getDbType();
		String colstr="";
		String mtmtable="";
		String mtmfilter="";
	...
	public void makeSQL(String table , String[] filter ,String [] column, String[] datatype,String accessfilter,QueryImpl impldata){
		String dbtype=DatabaseTransaction.getDbType();
		String colstr="";
		...
		for ( int i=0; i<k; ){
			state=0;
			criteria=filter[i].indexOf(".")>0||filter[i].indexOf("(")>=0 ?filter[i]:prefix+"."+filter[i];
			operator=filter[i+1];
			if (!search.equals(""))
				search+=" and ";
			boolean verifydate=filter[i].toUpperCase().indexOf("DATE")>0&&tu.isDate(filter[i+2]);
		...
		search=this.getAccessFilter(table,search,accessfilter, impldata,prefix);
		if(this.getIsForm()){
			if(tu.isEmptyValue(search)){
				search=" rownum < 2";
			}else{
	...
	//This method create the sql for child tables
	public  String  makeChildObjectFilterSql(String parent,String childname,String relfield,String pid,String[]column,String[]datatype,String objectfilter,String accessfilter,QueryImpl impldata){
		String dbtype=DatabaseTransaction.getDbType();
		String colstr="";
		String mtmtable="";
		String mtmfilter="";
		...
		for (int i=0;i<column.length;i++){
			...
			if(viewfldlist!=null &&viewfldlist.length>0)
				for(int k=0;k<viewfldlist.length;k++){
				  try{
					String[] items=viewfldlist[k].toLowerCase().split(" ");
					if (!viewfldlist[k].contains("(") &&tu.getString2TokenArray(viewfldlist[k].toLowerCase()," ")[1].equals(column[i].toLowerCase())){
						field=viewfldlist[k];
						break;
					}else if (items.length>0 &&items[items.length-1].contains(column[i].toLowerCase())){
						  field=viewfldlist[k].replaceAll("#", ",");
						  break;
					}
				  }catch(Exception e){
					  if (viewfldlist[k].toLowerCase().contains(column[i].toLowerCase())){
						  field=viewfldlist[k].replaceAll("#", ",");
						  break;
					  }
				  }
	...
	//This method converts the data datatype
	public void makeTableSelectObjectFilter(String table,String criteria , String operator ,String value,String [] column,String[] datatype,String objectfilter,String accessfilter,QueryImpl impldata)
	...
	//This method verify the object privilege based on different group
	//Additional group can be added later like team, manager etc based on the above steps
	public void makeObjectFilterSQL(String table , String[] filter ,String [] column, String[] datatype,String objectfilter,String accessfilter,QueryImpl impldata){
		String dbtype=DatabaseTransaction.getDbType();
		String colstr="";
		String prefix=table.toLowerCase();
		String filterstr[]=tu.getString2TokenArray(objectfilter,"@");
		...
		String viewfldlist[]=tu.getString2TokenArray(filterstr[0],",");
		String viewfldstr="";
		String tablealias=table.toLowerCase();
		...
		for (int i=0;i<column.length;i++){
			...
			if(viewfldlist!=null &&viewfldlist.length>0)
				for(int k=0;k<viewfldlist.length;k++){
                    ...
                    String[] items=tu.getString2TokenArray(viewfldlist[k].toLowerCase()," ");
					if (items!=null &&items.length>1 && items[1].equals(column[i].toLowerCase())){
						field=viewfldlist[k];
						break;
					}
				}
	...
	public  String makeBulkSQL(boolean isparent,String xml,	String relfield,QueryImpl impldata){
		String dbtype=DatabaseTransaction.getDbType();
		String sql=tx.makeBulkSQL(dbtype,isparent, xml, relfield,impldata);
	...
	public String removeSql(String parent, String parentObjId ,String[] childs, String groupuser){
		...
		if(childs!=null&&childs.length>0){
			for (String child:childs[0].split(",")){
